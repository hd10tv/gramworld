...
async def not_joined(client: Client, message: Message):
    # --- Tutorial Video Logic (Added) ---
    try:
        # Type handling for TUTORIAL_VIDEO_ID
        if isinstance(TUTORIAL_VIDEO_ID, str):
            video_id = int(TUTORIAL_VIDEO_ID)
        elif isinstance(TUTORIAL_VIDEO_ID, int):
            video_id = TUTORIAL_VIDEO_ID
        else:
            raise ValueError(f"TUTORIAL_VIDEO_ID must be a string or integer, not {type(TUTORIAL_VIDEO_ID)}")

        await client.send_video(
            chat_id=message.from_user.id,
            video=video_id,
            caption="Watch this tutorial before proceeding!",
        )
    except (ValueError, TypeError) as ve:
        await message.reply_text(f"Error with TUTORIAL_VIDEO_ID: {ve}")
        return  # Stop execution if there's a TUTORIAL_VIDEO_ID error
    except MessageNotModified:
        print("Tutorial video likely already sent.")
    except Exception as e:
        await message.reply_text(f"Error sending tutorial video: {e}")
        return  # Stop execution if there's a send_video error
...
@Bot.on_callback_query(filters.regex(r"^try_again$"))  # Add callback query handler
async def try_again_callback(client: Client, callback_query: CallbackQuery):
    if await subscribed(client, callback_query):
        ...
        # Resend the start message (if there's a command)
        if callback_query.message.reply_to_message and callback_query.message.reply_to_message.text.startswith("/start"):
           await start_command(client, callback_query.message.reply_to_message)
    else:
